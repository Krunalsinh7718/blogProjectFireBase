...
function PostCard({
  ...
  return (
    <>
      <Link to={`/blog/${slug}`}>
        <div
          className={`w-[300px] border shadow rounded-md hover:shadow-lg bg-white overflow-hidden app-blog-card ${className}`}
          {...props}
        >
          <LazyImage
          ...
          <div className="p-4">
            ...
            <h1
              className="text-lg font-semibold capitalize multiline-dotted-text mb-2"
              title={title}
            ...
            {createdTime ? (
              <span className="text-xs flex gap-2 items-center text-slate-500">
                <Icon.Calendar height={12} width={12} />
                <span>{timeStamptToDDMMYY(createdTime)} </span>
              </span>
            ) : null}
          </div>
        </div>
